        ...
        <div id="curriculum-preview" style="margin-top: 60px; text-align: left;">
          <div class="hero-stat-card" style="background: rgba(15, 23, 42, 0.85); padding: 32px;">
            ...
            <div style="max-height: 420px; overflow-y: auto; padding-right: 8px;">
              ${curriculum.map((x, i) => `
                <div style="padding: 12px 16px; border-bottom: 1px solid rgba(255,255,255,0.08); display: flex; justify-content: space-between; align-items: center; gap: 12px; flex-wrap: wrap;">
...
function companyBank(){
 ...
 ${companies.map(c=>`<div class="card"><h2>${c}</h2><div class="table-wrap"><table><thead><tr><th>#</th><th>Area</th><th>Question</th><th>Key Answer / Expected Point</th></tr></thead><tbody>${COMPANY_INTERVIEW_BANK[c].map((q,i)=>`<tr><td>${i+1}</td><td><span class="badge">${esc(q[0])}</span></td><td>${esc(q[1])}</td><td>${esc(q[2])}</td></tr>`).join("")}</tbody></table></div></div>`).join("")}</main>`;
}
...
function reviewHTML(a){
 ...
 <h3>MCQ Questions & Answers</h3><div class="table-wrap"><table><tr><th>Q</th><th>Question</th><th>Options</th><th>Student Answer</th><th>Correct Answer</th><th>Result</th></tr>${details.map(d=>`<tr><td>${d.number}</td><td>${esc(d.question)}</td><td>${esc(d.options.map((o,i)=>"ABCD"[i]+". "+o).join(" | "))}</td><td>${esc(d.answer)}</td><td>${esc(d.correct)}</td><td>${esc(d.result)}</td></tr>`).join("")}</table></div>
